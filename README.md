...
The owner explicitly excluded historical order `10C-00179` (Payload ID `160`) from repeat export on 2026-09-07. The retry service matches both identifiers and reports this order as skipped with a reason, including background retries, admin selections and `--force`. It neither modifies the order nor clears its stored error, and other uniqueness conflicts remain errors. Bulk totals include unchanged orders and exclusions in the skipped count, and show only actual export attempts as "к выгрузке". This exception requires only an application deployment, with no migration or environment changes; link recovery below remains available separately.
